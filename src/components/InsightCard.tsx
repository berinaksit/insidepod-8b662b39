...
import { FileText, TrendingUp, AlertTriangle, Sparkles, Activity, Target, Lightbulb, BarChart3, Users, Zap, LineChart, PieChart, MessageSquare, Clock, type LucideIcon } from 'lucide-react';
...
const agentIcons: Record<string, {
  icon: LucideIcon;
  name: string;
}> = {
  'risk-scanner': {
...
export function InsightCard({
  ...
  const getIcon = (): {
    ...
    if (agentType && agentIcons[agentType] && !usedIcons.has(agentIcons[agentType].name)) {
      return {
        Icon: agentIcons[agentType].icon,
        iconName: agentIcons[agentType].name
      };
    }
  ...
  if (layoutVariant === 0) {
    return <motion.article initial={{
      ...
      delay: index * 0.08,
      ease: [0.4, 0, 0.2, 1]
    }} className="bg-foreground rounded-xl p-4 sm:p-6 cursor-pointer group min-h-[220px] sm:min-h-[260px] flex flex-col" onClick={handleClick}>
  ...
  if (layoutVariant === 1) {
    return <motion.article initial={{
      ...
      delay: index * 0.08,
      ease: [0.4, 0, 0.2, 1]
    }} className="insight-card cursor-pointer group min-h-[220px] sm:min-h-[260px] flex flex-col" onClick={handleClick}>
        <div className="flex items-center gap-2 mb-4">
          <span className="text-sm text-[#b0b0b0] font-semibold">Ask</span>
          {insight.isNew && <span className="w-1.5 h-1.5 rounded-full bg-[#b0b0b0]" />}
        </div>
  ...
  return <motion.article initial={{
    ...
    ease: [0.4, 0, 0.2, 1]
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {insight.isNew && <span className="w-1.5 h-1.5 rounded-full bg-foreground/60" />}
      </div>
      ...
      <div className="mt-6 sm:mt-8">
        <div className="flex items-center gap-2 mb-3">
          <div className="flex-1 h-2 bg-muted/50 rounded-full overflow-hidden">
            <div className="h-full bg-foreground/30 rounded-full" style={{
