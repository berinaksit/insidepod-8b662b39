      ...
      <form onSubmit={handleSubmit}>
        <div className="search-container">
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            {!query && !isFocused && (
              <kbd className="hidden sm:inline-flex px-2 py-0.5 text-xs text-muted-foreground bg-muted rounded">
                /
              </kbd>
            )}
          </div>
        ...
        <AnimatePresence>
          {documents.map((doc) => (
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.9 }}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-muted/70 rounded-full group"
            >
      ...
      <AnimatePresence>
        {isFocused && !query && (
          <motion.div
            ...
            className="absolute top-full left-0 right-0 mt-2 bg-card rounded-lg shadow-card-hover border border-border overflow-hidden z-50"
          >
            <div className="p-2">
              <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1 px-2">
                Suggestions
              </p>
              <div className="space-y-0.5">
                {suggestions.map((suggestion, i) => (
                  <button
                    ...
                    className="w-full text-left px-2 py-2 text-sm text-foreground rounded-md hover:bg-muted transition-colors"
                  >
                    {suggestion}
                  </button>
                ))}
